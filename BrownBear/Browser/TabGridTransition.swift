...
final class TabGridTransitionAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    private func animateHeroExpand(_ image: UIImage, using context: UIViewControllerContextTransitioning) {
        ...
        // the tapped card's picture to the content-area frame (not the whole screen), landing 1:1 on the live
        // page beneath for a seamless dissolve. scaleAspectFill keeps it undistorted; since the snapshot and
        // the content area share an aspect, it fills the final frame exactly.
